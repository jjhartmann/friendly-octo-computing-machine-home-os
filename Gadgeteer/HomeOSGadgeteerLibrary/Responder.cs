...
namespace HomeOSGadgeteer.Networking
{
    ...
    public class Responder
    {
        ...
        internal void SendResponse()
        {
            try
            {
                ...
                string header = "";
                var content = webEvent.ResponseData;
                var contentType = webEvent.ContentType ?? "";
                if (content != null && content.Length == 0) content = null;
                var refreshAfter = webEvent.refreshAfter;
                ...
                var headerbytes = UTF8Encoding.UTF8.GetBytes(header);

                if (headerbytes != null && headerbytes.Length > 0)
                {
                    this.ClientSocket.Send(headerbytes, 0, headerbytes.Length, SocketFlags.None);

                    if (content != null && content.Length > 0)
                    {
                        this.ClientSocket.Send(content, 0, content.Length, SocketFlags.None);
                    }
                }
            ...
            for (int i = 0; i < splittedRequest.Length; i++)
            {
                ...
                if (line.Length > 0 && !line.Equals(" "))
                {
                    ...
                    if (i == 0)
                    {
                        ...
                        for (int c = 0; c < Path.Length - 2; c++)
                        {
                            if (Path[c] == '%')
                            {
                                int charint = 0;
                                for (int charindex = 1; charindex <= 2; charindex++) 
                                {
                                    if (Path[c + charindex] >= '0' && Path[c + charindex] <= '9') 
                                    {
                                        charint += (charindex == 1 ? 16 : 1) * (Path[c + charindex] - '0');
                                    }
                                    else if (Path[c + charindex] >= 'a' && Path[c + charindex] <= 'f')
                                    {
                                        charint += (charindex == 1 ? 16 : 1) * (Path[c + charindex] - 'a' + 10);
                                    }
                                    else if (Path[c + charindex] >= 'a' && Path[c + charindex] <= 'f')
                                    {
                                        charint += (charindex == 1 ? 16 : 1) * (Path[c + charindex] - 'A' + 10);
                                    ...
                                    {
                                        Debug.Print("ERROR: percent character in URL but subsequent 2 characters are not hex digits - not percent decoding");
                                        continue;
                                    }
                                }
                                Path = Path.Substring(0, c) + ((char)charint) + Path.Substring(c + 3);
                                Debug.Print("Decoded percent encoding in URL position " + c + " : " + Path);
                            }
                        }
